...
public class Munch {
  ...
  /**
   * This takes in an int which determines how many iterations to run the Hill climbing algorithm for and 2D array of
   * integers(Module Dependency Graph). It finds the clusters in the MDG and outputs how the node/elements are grouped
   * together to form a cluster. <br> It returns an {@code ArrayList<Integer>} which contains clustering arrangement
   * <br> e.g. {@code C = [1,2,2,3,3,3,4,4,4,4]} (length = 10) which means that there are <br> total of 4 clusters and
   * object at C_1 is in cluster 1, C_2 is in cluster 2, C_3 = 2 and C_10 = 4.<br>
   *
  ...
  /**
   * Checks if the given 2D array is a valid MDG.<br> It checks for null, empty, equal length, valid data and symmetry.
   *
